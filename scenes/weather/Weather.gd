...
@export var snow_darkness = 0.2 # (float, 0, 1)
@export var rain_darkness = 0.3 # (float, 0, 1)
...
func change_weather():
	...
	if weatherType == 'clear':
		apply_rain_settings()
		apply_snow_settings()
		#if delayWeatherChange: await tween.tween_completed # Waits weather change before turning off emission and lower light
	...
	if weatherType == 'snow':
		...
		#if delayWeatherChange: await tween.tween_completed # Waits light change to change weather
...
func change_size(weather, new_size):
	...
	#Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	#tween.start()
	#weather.process_material.anim_offset = new_size
	pass
...
func change_wind_direction(weather, new_wind):
	...
	#Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	#tween.start()
	weather.process_material.direction.x = new_wind
